...
CREATE TABLE Admin(adminId INTEGER, name VARCHAR(40), password VARCHAR (40), PRIMARY KEY (adminId));
...
CREATE TABLE Verifies (adminId INTEGER, governmentId VARCHAR(30),PRIMARY KEY (governmentId),
FOREIGN KEY (adminId) REFERENCES Admin ON DELETE SET NULL,
FOREIGN KEY (governmentId) REFERENCES Host ON DELETE CASCADE);
	
CREATE TABLE Location (postalCode CHAR(6), city VARCHAR (30), country VARCHAR(30),PRIMARY KEY (postalCode)); 

CREATE TABLE ListingPostedIsIn (listingId CHAR (10), price REAL, capacity INTEGER, private CHAR(1),rating REAL, 
governmentId VARCHAR (30) NOT NULL, postalCode CHAR(6) NOT NULL, address VARCHAR(40),PRIMARY KEY (listingId),
FOREIGN KEY(governmentId) REFERENCES Host ON DELETE CASCADE, 
FOREIGN KEY(postalCode) REFERENCES Location, 
CHECK (price > 0));    

CREATE TABLE AmenitiesIncluded (listingId CHAR(10), amenityId CHAR(5),tv CHAR(1), kitchen CHAR(1), internet CHAR(1), 
...
CREATE TABLE ListingIdAndEmail (listingId CHAR(10), email VARCHAR (40) NOT NULL, PRIMARY KEY (listingId), 
FOREIGN KEY (listingId) REFERENCES ListingPostedIsIn ON DELETE CASCADE,
...
FOREIGN KEY (reservationId) REFERENCES FulfillsReservation ON DELETE CASCADE, 
CHECK (ratingStars >= 1 AND ratingStars <=5));

CREATE TABLE CreditCardInfo(cardNumber CHAR (15), company CHAR (20), cardHolderName CHAR (20), expirationDate DATE,             
PRIMARY KEY (cardNumber));

CREATE TABLE CreditCardTransaction(transactionId CHAR(10), cardNumber CHAR(15),PRIMARY KEY (transactionId),
FOREIGN KEY (cardNumber) REFERENCES CreditCardInfo ON DELETE CASCADE,
FOREIGN KEY (transactionId) REFERENCES Transaction ON DELETE CASCADE);
